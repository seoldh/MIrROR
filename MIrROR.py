...
if __name__ == "__main__":
    ...
    parser.add_option("-d", "--db_dir", dest="databasefolder", action="store",  help="directory containing MIrROR database [required]", metavar="DBDIR")
    parser.add_option("-o", "--output_dir", dest="outputfolder", action="store", default='./Result', help="specify directory to output files (default : ./Result)", metavar="OUTDIR")
    ...
    mapping_opts.add_option("-M", "--minibatch", dest="minibatch", action="store", default="500M", help="number of query bases loaded to the memory at once. K/M/G/k/m/g suffix is accepted. (default : 500M)", metavar="NUM")
    ...
    outformat_opts.add_option("-m", "--residuemaches", dest="residuemaches", action="store", default=2500, type='int',  help="number of residue matches (default : 2500)", metavar="INT")
    outformat_opts.add_option("-b", "--blocklength", dest="blocklength", action="store", default=3500, type='int',  help="alignment block length (default : 3500)", metavar="INT")
    ...
    graph_opts.add_option("-S", "--Stackedplot", dest="stackedplot", action="store_true", help="create a stacked bar plot. Requires Python packages to be installed")
    ...
    help_opts.add_option("-v", "--version", dest="version", action="store_true", help="show version number and exit\n")
